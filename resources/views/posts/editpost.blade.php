        ...
        <div class="row align-items-center d-flex justify-content-center" style="min-height: 100vh">
            <div class="col-md-10">
                <form action="/api/posts/{{$post->id}}" method="POST">
                    ...
                    @method('PUT')
                    <h3>Назва</h3>
                    <input type="text" class="form-control" id="postname" name="name" placeholder="Новина" value="{{ $post->name }}">
                    <h3>Короткий опис</h3>
                    <input type="text" class="form-control" id="postdescription" name="description" placeholder="В цьому пості ви хочете описати..." value="{{ $post->description }}">
                    <h3>Текст поста</h3>
                    ...
                    <a id="deletebutton" class="btn btn-danger" onclick="event.preventDefault(); document.getElementById('delete-form').submit();">Видалити</a>
                </form>
